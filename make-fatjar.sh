#!/bin/sh
# Build a single self-contained Tyler.jar: the modern Swing UI (TylerSwing) with
...
cp build_classes/*.class fatjar/           # add Tyler's default-package classes
...
echo "done  ->  Tyler.jar     (run: java -jar Tyler.jar   |   dark: java -jar Tyler.jar -dark)"
